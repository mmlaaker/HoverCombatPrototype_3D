...
1. **Express knockback as a fraction of top speed, not as a raw number.** A hit that imparts more than top speed is not disruption, it is removal: the target is travelling faster than it can drive and its own momentum skill is irrelevant until it stops.
   ...
   **Re-baselined against topSpeed 105 and judged in play 2026-08-24. The Rocket Launcher is the calibration point for the roster.** A direct hit imparts **132 m/s, 1.26x top speed** -- and since a boosted craft tops out at 131.25, that means *a rocket throws you at almost exactly your own maximum speed*. It travels roughly 127m before the hover arrests it.

   *This rule was originally authored against a top speed of 60. The speed pass moved it to 105 and nothing rechecked the roster, so every weapon quietly changed meaning: the rocket's old force went from 1.67x (removal) to 0.95x (disruption) without a value moving. That is the failure this section exists to prevent, and it is recorded in `TODO.md` 5.16.*
2. **Where you hit matters more than how hard.** The chassis rolls about three times more easily than it pitches, so a hit that catches a flank high tips the craft far more readily than a square hit of the same force. Re-measured 2026-08-24 with the probe: a centred hit tilted the target **0.9 degrees** and it drove on; the same shot landing high on the flank rolled it **179.7 degrees** and downed it. **The shove was identical in both cases -- 100.0 m/s.** Only the rotation differed, which is `WeaponImpact`'s split doing exactly what it claims: hit location decides tumble, force decides distance, and the two do not interfere.
